...
struct __packed__{
  ...
  uint8 led:1;                 //bit 0
}*IOp_A=(void*)(GPIO1_BASE_ADDRESS+GPIO1_PA);
...
}*IOp_B=(void*)(GPIO1_BASE_ADDRESS+GPIO1_PB);

struct __packed__{
  uint8 res:6;                 //bit 2..7
  uint8 dac_clr:1;
  uint8 dac_ldac:1;            //bit 0
}*IOp_C=(void*)(GPIO1_BASE_ADDRESS+GPIO1_PC);
...
}*IOp_D=(void*)(GPIO1_BASE_ADDRESS+GPIO1_PD);
...
void init_Encoders(void){
  enc[0]=(int32_t*)WB_ENCODER_1_BASE_ADDRESS;
  enc[1]=(int32_t*)WB_ENCODER_2_BASE_ADDRESS;
  enc[2]=(int32_t*)WB_ENCODER_3_BASE_ADDRESS;
  for(uint8 i=0;i<3;i++)*enc[i]=0;
}

// ADCs
uint8_t *adc_addr=(void*)(GPIO1_BASE_ADDRESS+GPIO1_PE);
uint16_t *ADC_X_0;
uint16 ADC;
uint16 power24;
...
void init_ADCs(void){
  *adc_addr=0;
  ADC_X_0=(void*)(WB_ADCX_BASE_ADDRESS);
}
...
//    Rb.enc_position_1=*(int16_t*)Base_WB_ENCODER_1;
//    Rb.enc_position_2=*(int16_t*)Base_WB_ENCODER_2;
//    Rb.enc_position_3=*(int16_t*)Base_WB_ENCODER_3;
    encoder=(uint16_t)*enc[0];
    Rb.enc_position_1=htoes(encoder);
    encoder=(uint16_t)*enc[1];
    Rb.enc_position_2=htoes(encoder);
    encoder=(uint16_t)*enc[2];
    Rb.enc_position_3=htoes(encoder);
/*
    for(uint8 i=0;i<3;i++){
