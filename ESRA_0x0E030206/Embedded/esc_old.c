...
// WARNING!!! special hack!
// dsPIC has issues with unaligned 16 bit copies, so use 8bits instead
// look for p8 in the code
...
int16 SDO_findobject(uint16 index)
{
  int16 n = 0;
...
void ESC_ALstatus(uint8 status)
{
  ...
  ESC_write(ESCREG_ALSTATUS, &dummy, sizeof(dummy), (void *)&ESCvar.ALevent);
}
...
{
  ESC_read(ESCREG_ADDRESS, (void *)&ESCvar.address, sizeof(ESCvar.address), (void *)&ESCvar.ALevent);
...
void ESC_readmbx(void)
{
  ...
  ESC_read(MBX0_sma + MBXHSIZE, &(MB->b[0]), length, (void *)&ESCvar.ALevent);
...
uint8 ESC_pre_objecthandler(uint16 index, uint8 subindex)
{
...
void ESC_stopoutput(void)
{
  ...
  ESC_SMdisable(2);
// TODO set outputs to safe position
}
